...
public:
   HNF_companion_logger(SparseMatrix<E> *Larg, SparseMatrix<E> *Rarg) : L(Larg), R(Rarg) {}
...
int hermite_normal_form(SparseMatrix<E>& M, std::list< std::pair<E,int> >& torsion,
...
#if POLYMAKE_DEBUG
   const bool do_dump=getenv("SMITH_DUMP");
   if (do_dump) cout << "eliminate_ones(initial):\n" << std::setw(6) << M;
   pm::SparseMatrixStatistics<E> stat;
   stat.gather(M);
...
#if POLYMAKE_DEBUG
      stat.gather(M);
      if (do_dump) cout << "eliminate_ones(eliminating col " << c << " [" << r << "] ):\n" << std::setw(6) << M;
#endif
      c_last=c++;
   }

#if POLYMAKE_DEBUG
   cout << "eliminate_ones(final statistics):\n" << stat << endl;
#endif
   return count;
}
                  ...
                  typename enable_if<bool, std::numeric_limits<E>::is_integer>::type inverse_companions=false)
{
   ...
   if (inverse_companions)
      res.rank=hermite_normal_form(res.form, res.torsion, HNF_companion_logger<E, false>(&res.left_companion, &res.right_companion), True());
   else
      res.rank=hermite_normal_form(res.form, res.torsion, HNF_companion_logger<E, true>(&res.left_companion, &res.right_companion), True());
